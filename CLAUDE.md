...
- `Input/MapHotkeyPoller.cs`, `Input/CombatHotkeyPoller.cs`, `Input/EventHotkeyPoller.cs`,
  ...
  `Input/FinishRunHotkeyPoller.cs`, `Input/CharWindowHotkeyPoller.cs` — `MonoBehaviour`s that poll letters the game leaves
...
CharWindow > Combat > Event > TownUpgrade > Town > Map > Rewards > Loot > FinishRun > CharPopup >
