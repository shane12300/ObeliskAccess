...
- `RouterInputPatches.cs` — **the ONLY patches on `InputController`.** `DoMovement` (prefix,
  ...
  **The game does NOT ignore Tab**: `DoKeyBinding`
  ...
  "random MP lockup"; Escape cured it via `CancelTyping`). The prefix therefore swallows Tab
  whenever any context is active (`InputRouter.HasActiveContext`), and a `ChatManager.EnableChat`
