...
Event > TownUpgrade > Town > Map > Rewards > Loot > FinishRun > PerkTree > CharPopup >
HeroSelection > MainMenu`. A modal thus sits above the screen beneath it. (Matches the game's own
modality order `… Event > Town > Map > Rewards …`; CardCraft sits above Event because event shops
open over the map; DeathScreen and CombatSelector are the in-combat modals over Combat; the
hero-selection family stacks PerkTree over CharPopup over the base screen, per the game's
`PerkTree > … > HeroSelection/CharPopup` order — note this places the PerkTree context BELOW
Town, so it only drives the tree in the HeroSelection scene, not the town-tier-0 usage (todo);
Alert outranks everything — the game's confirm dialogs are modal over every screen.) All three
hero-selection contexts go inert while the madness or sandbox panel is open (deferred interiors —
the game's own controller nav drives those).
